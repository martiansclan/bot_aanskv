...
class ServiceRegistry {
    ...
    async initialize(app) { // <-- ДОБАВЛЕН ПАРАМЕТР app
        if (this.initialized) {
        ...
        try {
            ...
            // ========== ИНИЦИАЛИЗАЦИЯ БОТА С ПЕРЕДАЧЕЙ APP ==========
            try {
                const botService = require('../modules/bot/bot.service');
                
                // Проверяем, есть ли метод initialize()
                if (typeof botService.initialize === 'function') {
                    // ВАЖНО: Передаем app в метод initialize!
                    const botInitialized = await botService.initialize(app);
                    if (botInitialized) {
                        this.register('bot', botService);
                        logger.success('Сервис бота инициализирован');
                    } else {
                        logger.warning('Сервис бота не инициализирован, но доступен');
                        this.register('bot', botService);
                    }
                } else {
                    this.register('bot', botService);
                    logger.info('Сервис бота зарегистрирован (без метода initialize)');
                }
            } catch (error) {
                logger.error('Ошибка инициализации модуля бота:', error.message);
                try {
                    const botService = require('../modules/bot/bot.service');
                    this.register('bot', botService);
                    logger.info('Сервис бота зарегистрирован (несмотря на ошибку инициализации)');
                } catch (regError) {
                    logger.error('Не удалось зарегистрировать сервис бота:', regError.message);
                }
            }
            // ========== КОНЕЦ ИНИЦИАЛИЗАЦИИ БОТА ==========
    ...
    async shutdown() {
        ...
        try {
            ...
            if (botService && typeof botService.shutdown === 'function') {
        ...
        // <-- ИСПРАВЛЕНО: очищаем сервисы
        try {
            this.services.clear();
            this.initialized = false;
            logger.success('Сервисы остановлены');
        } catch (error) {
            logger.error('Ошибка при очистке сервисов:', error.message);
        }
    }
}
