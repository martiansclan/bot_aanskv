...
class TribeInfoCollectorWebSocketManager {
    constructor() {
        this.wss = null;
    ...
    initialize(server) {
        this.wss = new WebSocket.Server({ 
            server,
            path: '/api/TribeInfoCollector/ws'
        });
        
        this.setupWebSocket();
        console.log('✅ WebSocket сервер Tribe Info Collector запущен на /api/TribeInfoCollector/ws');
    }
    
    setupWebSocket() {
        this.wss.on('connection', (ws, req) => {
            console.log('🔌 Новое WebSocket соединение Tribe Info Collector');
            ...
            ws.on('error', (error) => {
                console.error('WebSocket ошибка Tribe Info Collector:', error);
            });
        });
    }
    ...
    completeCalculation(calculationId, result) {
        ...
        // Через 5 минут очищаем старые данные
        setTimeout(() => {
            this.cleanupCalculation(calculationId);
        }, 5 * 60 * 1000);
    }
    ...
    failCalculation(calculationId, error) {
        ...
        // Через 5 минут очищаем
        setTimeout(() => {
            this.cleanupCalculation(calculationId);
        }, 5 * 60 * 1000);
    }
    
    // Очистка данных расчета
    cleanupCalculation(calculationId) {
        this.clients.delete(calculationId);
        this.calculationProgress.delete(calculationId);
        console.log(`🧹 Очищены данные расчета ${calculationId}`);
    }
...
module.exports = new TribeInfoCollectorWebSocketManager();